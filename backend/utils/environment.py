...
def get_environment(
    name: str, mapper: Optional[Callable[[str], Any]] = None, default=NOT_SET
) -> Any:  # noqa
    """Return a value with the given name from the process environment variables"""
    possible_environ_names = [f"{name}", name]
    for environ_name in possible_environ_names:
        ...
        if default is NOT_SET:
            raise ImproperlyConfigured(
                f"The {name} environment variable is not present "
                f"under names: {', '.join(possible_environ_names)}"
            )
        else:
            return default
...
def boolean_mapper(value: str) -> bool:
    ...
    if value.lower() in {"false", "0", "no"}:
        return False
    raise ImproperlyConfigured(f"Boolean mapper cannot map the {value} value to bool")


def list_mapper_factory(
    delimiter: Optional[str] = None, item_mapper: Optional[Callable[[str], Any]] = None
) -> Callable[[str], List[Any]]:  # noqa
    """Return a function that maps lists serialized to a string
    with a specified delimiter to a Python list
    ...
    def list_mapper(value: str) -> List[Any]:
        ...
        Assumes an empty string represents an empty list,
